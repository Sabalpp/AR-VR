...
config['properties'].update({'target_repetitions':{'type':'integer','minimum':1,'maximum':100},'authoritative_counter':{'const':'backend'}})
...
server_payloads = {
    'session.config':obj({'config':{'$ref':'#/$defs/ExerciseConfig'},'mode':{'enum':['phone','quest']},'authoritative_counter':{'const':'backend'},'device_id':string,'last_seq':{'type':'integer','minimum':-1},'server_time':{'type':'string','format':'date-time'}}),
    ...
    'exercise.feedback':obj({'message':string,'cue_id':{'enum':['reach','tracking_lost']}}),
